<!-- From https://github.com/lucien144/vue3-parallaxy/blob/main/src/components/Parallaxy.vue -->
<template>
  <div
    :class="[
      'parallaxy',
      {
        'parallaxy--loaded': loaded,
        'parallaxy--disabled': options.disabled,
        'parallaxy--debug': options.debug
      }
    ]"
    :style="_animation"
    ref="$outer"
...
const _animation = computed(() => {
  ...
  if (options.axis === 'y') {
    return options.animation
      ? options.animation(delta.value)
      : `transform: translate3d(0, ${delta.value}px, 0);`
  } else {
    return options.animation
      ? options.animation(delta.value)
      : `transform: translate3d(${delta.value}px, 0, 0);`
  }
})
const directionVector = computed(() => (['normal'].includes(options.direction ?? '') ? 1 : -1))
const calculateDelta = () =>
  (delta.value =
    (window.innerHeight - ($outer.value?.getBoundingClientRect().top ?? 0)) *
    ((options.speed as number) / -1000) *
  ...
  if (props.breakpoints !== undefined) {
    ...
    for (const breakpoint of breaks) {
      if (window.innerWidth >= Number.parseInt(breakpoint)) {
        options.axis =
          (props.breakpoints as Array<ParallaxyOptions>)[Number.parseInt(breakpoint)]?.axis ||
          options.axis
        options.speed =
          (props.breakpoints as Array<ParallaxyOptions>)[Number.parseInt(breakpoint)]?.speed ||
          options.speed
        options.direction =
          (props.breakpoints as Array<ParallaxyOptions>)[Number.parseInt(breakpoint)]?.direction ||
          options.direction
        options.animation =
          (props.breakpoints as Array<ParallaxyOptions>)[Number.parseInt(breakpoint)]?.animation ||
          options.animation
        options.disabled =
          (props.breakpoints as Array<ParallaxyOptions>)[Number.parseInt(breakpoint)]?.disabled ||
          options.disabled
        options.debug =
          (props.breakpoints as Array<ParallaxyOptions>)[Number.parseInt(breakpoint)]?.debug ||
          options.debug
        break
      }
    }
